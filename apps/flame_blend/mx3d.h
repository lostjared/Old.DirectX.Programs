...
public:
	...
	void display_scale(float x, float y, float scale_x, float scale_y, float rotation) {
		...
		D3DXMatrixTransformation2D(&mat,NULL,0.0,&scaling,&sprite_center,rotation,&trans);
	...
	void display_ex(float x, float y, int x1, int y1, int x2, int y2, float scale_x, float scale_y, float rotation) {
		...
		D3DXMATRIX mat;
		D3DXVECTOR2 scaling(scale_x,scale_y);
		D3DXMatrixTransformation2D(&mat,0,0.0,&scaling,&sprite_center,rotation,&trans);
		sprite->SetTransform(&mat);
...
public:
	...
	void __fastcall drawline(int start_x, int start_y, int stop_x, int stop_y,COLORREF color);
};
