...
int XSTD MasterMain(HINSTANCE hInst,LPSTR line)
{
	srand(time(NULL));
	loadscore();
	if(mxhwnd.CreateMasterX("MasterPeiece",640,480,COLOR_DEFAULT,MasterProc,GetModuleHandle(NULL),LoadIcon(GetModuleHandle(0), MAKEINTRESOURCE(IDI_ICON1)),LoadCursor(0,IDC_ARROW)))
	{
		onload();
